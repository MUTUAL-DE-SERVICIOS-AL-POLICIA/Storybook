...
export const SpinnerLoading = ({
  ...
  return (
    <>
      {isTextTop &&
        <div className={`absolute left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-10 ${topTextTop}`}>
      ...
      {!notSpinner && 
        <div className={`absolute left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-10 ${topSpinner}`}>
      ...
      {isTextBottom &&
        <div className={`absolute left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-10 ${topTextBottom}`}>
          <span className="text-2xl font-semibold text-default-900 bg-white dark:bg-black rounded-sm min-w-[1000px] text-center">
            {labelBottom}
          </span>
        </div>
